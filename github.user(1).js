...
// ========== 解密功能函数 ==========
async function uploadToDecodeInput() {
    if (!isAuthenticated()) {
    ...
    try {
        ...
        // 获取当前选择的分支（从下拉框或存储的值）
        const branchSelect = document.getElementById('__gh_branch_select__');
        let targetBranch = branchSelect ? branchSelect.value : info.branch;
        ...
        // 获取文件内容
        const content = await getRawFileContent(info.owner, info.repo, targetBranch, info.filePath);
        ...
        const uploadData = {
            message: `Update ${decodeRepo.inputFile} via GitHub助手`,
...
if (isForkRepository()) {
  ...
  try {
    ...
    // 上传到decode库的input.js
    const success = await uploadToDecodeInput(content);
    if (success) {
...
// 作者信息 - 添加在面板最底部
  ...
  authorText.textContent = 'GitHub助手增强版 by Mr.Eric 6.0.27';
